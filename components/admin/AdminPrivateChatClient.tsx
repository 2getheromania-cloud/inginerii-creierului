...
export default function AdminPrivateChatClient({ conversationId, adminId }: Props) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      {error && (
        <div className="px-3 py-2 text-sm text-red-600 bg-red-50 border-t border-red-100">
          {error}
          <button onClick={() => setError(null)} className="ml-2 underline text-xs">ok</button>
        </div>
      )}
      <form onSubmit={handleSend} className="border-t border-gray-200 px-3 py-2 flex gap-2 items-end flex-shrink-0">
        <textarea
          value={text}
          onChange={e => { setText(e.target.value); const el = e.target; el.style.height='auto'; el.style.height=Math.min(el.scrollHeight,80)+'px' }}
          onKeyDown={e => { if (e.key==='Enter'&&!e.shiftKey){e.preventDefault();handleSend(e as unknown as React.FormEvent)} }}
          placeholder="Scrie un mesaj..."
          rows={1}
          className="flex-1 resize-none rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400"
          style={{ minHeight:'36px', maxHeight:'80px' }}
        />
        <button type="submit" disabled={!text.trim()||sending}
          className="bg-brand-600 text-white rounded-full w-8 h-8 flex items-center justify-center flex-shrink-0 disabled:opacity-40 hover:bg-brand-700 transition-colors">
